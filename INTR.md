...
**批量删除**：空格、逗号、顿号分隔多个序号（越界序号自动忽略，不存在的序号不报错）：
...
到点后机器人会在群内发送提醒并 @ 相关人员：个人待办 @ 创建者，团体待办 @ 参与成员与发起人。待办保存在 `data/todos.json`，重启后自动恢复（以群名匹配回退，群 id 变化也不影响查看/完成/删除）。完成的待办会立即从待办文件与列表中清除，文件仅保留未完成待办。
