...
- **待办提醒**：@ 机器人发送"添加待办 [时间] 内容"创建待办，到点自动提醒。支持个人待办（@ 创建者）与团体待办（@ 消息中指定的参与成员）。支持"查看待办/完成待办/删除待办"管理。运行期间持续生效，重启后自动恢复。
...
```
@机器人 添加个人待办 明天9点 交周报
@机器人 添加团体待办 明天15点 开会 @张三 @李四
@机器人 查看待办
@机器人 完成待办 1
@机器人 删除待办 2
```

时间支持 `明天9点`、`今天18:30`、`30分钟后`、`2小时后`、`后天`、`8月25日 14:30`（指定日期时刻，可带年份如 `2027年8月25日 14:30`；无年份时日期已过自动顺延至明年）等描述；不带时间时只记录不提醒。待办保存在 `data/todos.json`，重启后自动恢复。
